...
# Facendo un join ottengo un df più completo
...
# Troviamo i generi di esemplare più presenti nel DB
temp_df = full_df %>%
  count(genere) %>%
  filter(n > 75)
...
# Troviamo i generi di esemplare meno presenti nel DB
temp_df = full_df %>%
  count(genere) %>%
  filter(n < 55)
...
# Analizziamo ora tutti gli stipendi, di seguito il codice per ottenere istogramma e densità
...
# In che anni sono state effettuate più visite?
