...
esemplare.data_nascita = sample(seq(as.Date('1999/01/01'), as.Date('2020/01/01'), by="day"), 4500)

# Creo un vettore per la data di arrivo dell'esemplare date successive randomiche a quelle di nascita
vettore_giorni = 1:365 # circa 1 anno
...
visita.esemplare_id = rep(vettore_esemplare$id, 60)         # ottengo vettore lungo 270k
visita.esemplare_genere = rep(vettore_esemplare$genere, 60) # ottengo vettore lungo 270k

length(visita.esemplare_id)
length(visita.esemplare_genere)

# data
vettore_esemplare_data_arrivo = dbGetQuery(con, "select data_arrivo from esemplare")
vettore_esemplare_data_arrivo = rep(vettore_esemplare_data_arrivo$data_arrivo, 60)
min_data = max(vettore_esemplare_data_arrivo)

visita.data = sample(seq(min_data, as.Date('2030/01/01'), by="day"), 270000, replace=TRUE)
